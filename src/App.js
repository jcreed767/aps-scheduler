...
// ─── Inline Logo (avoids public folder path issues) ───────────────────────────
...
export default function App() {
  ...
  // Manual schedules for sales/collections/districts
  ...
  const handleGenerate = useCallback(() => {
    const s = generateRotation(teams.callCenter, startDate, endDate, overrides);
    setSchedule(s); setGenerated(true);
  }, [teams.callCenter, startDate, endDate, overrides]);

  const coverage = useMemo(() => {
    if (!schedule) return {};
    return analyzeCoverage(schedule, teams.callCenter, forecastData);
  }, [schedule, teams.callCenter, forecastData]);

  const managerSummary = useMemo(() => {
    if (!schedule) return [];
    return getManagerSummary(schedule, teams.callCenter);
  }, [schedule, teams.callCenter]);
  ...
  const updateMember = (teamKey, id, field, value) =>
    setTeams(p => ({ ...p, [teamKey]: p[teamKey].map(m => m.id===id ? {...m,[field]:value} : m) }));

  const addMember = (teamKey, role) => {
    const id = `${teamKey}-${Date.now()}`;
    setTeams(p => ({ ...p, [teamKey]: [...p[teamKey], { id, name: `New ${role}`, role, team: teamKey, sites: '', shiftAnchor: null, unavailable: [], rotationOffset: undefined }]}));
  };
  ...
  const handleExport = () => {
    ...
    const gridRows = [['Date','Day','DOW',...teams.callCenter.map(m=>m.name),'A','B','C','Total','Meets Target','Forecast']];
    dates.forEach(date => {
      const dk = getDateKey(date); const ds = schedule[dk]||{}; const cov = coverage[dk]||{};
      gridRows.push([dk,format(date,'MMM d'),DOW_LABELS[getDay(date)],...teams.callCenter.map(m=>ds[m.id]||'OFF'),cov.counts?.A||0,cov.counts?.B||0,cov.counts?.C||0,cov.totalWorking||0,cov.meetsTarget?'Yes':'No',cov.forecastCalls||'']);
    });
    ...
    managerSummary.forEach(s => {
      const m = teams.callCenter.find(x=>x.id===s.id);
  ...
  function getDaySnapshot(dateKey, teamKey) {
    if (teamKey === 'callCenter') {
      ...
      return teams.callCenter.filter(m => ds[m.id] && ds[m.id] !== 'OFF').map(m => ({ ...m, shift: ds[m.id] }));
    }
    const teamKey2 = teamKey === 'sales' ? 'sales' : teamKey === 'collections' ? 'collections' : 'districts';
    const ms = manualSchedules[teamKey2]?.[dateKey] || {};
    return teams[teamKey].filter(m => ms[m.id] && ms[m.id] !== 'OFF').map(m => ({ ...m, shift: ms[m.id] }));
  }
          ...
          {page === 'dashboard' && (
            <div>
                    ...
                    return (
                      <div key={tk} style={{marginBottom:16}}>
                        <div className="team-coverage-item" style={{marginBottom:6}}>
                          ...
                          <span className="team-staff-count">{onShift.length} / {teams[tk].length} on shift</span>
                        </div>
              ...
              <div className="stats-row">
                <div className="stat-card"><div className="stat-value">{teams.callCenter.length + teams.sales.length + teams.collections.length + teams.districts.length}</div><div className="stat-label">Total Team Members</div></div>
              ...
              {!generated ? (
                ...
                <>
                  <div className="schedule-meta">
                    ...
                    <span className="meta-item"><strong>{teams.callCenter.length}</strong> managers</span>
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {managerSummary.map(s=>{
                      const mgr=teams.callCenter.find(m=>m.id===s.id);
                      return (
                        <tr key={s.id}>
                          ...
                          <td>{mgr?.shiftAnchor ? <ShiftBadge shift={mgr.shiftAnchor}/> : <span className="text-muted">Flex</span>}</td>
                        </tr>
                      );
                    })}
                  </tbody>
              ...
              {Object.entries({
                ...
                <div className="settings-section" key={tk}>
                  <div className="section-header-row">
                    <h3>{meta.label} ({teams[tk].length})</h3>
                      ...
                      {teams[tk].map(m=>(
                        <tr key={m.id}>
                          ...
                          {tk==='callCenter' && (
                            <>
                              <td>
                                <select className="inline-select" value={m.shiftAnchor||''} onChange={e=>updateMember(tk,m.id,'shiftAnchor',e.target.value||null)}>
                                  <option value="">Flexible</option>
                                  {['A','B','C'].map(s=><option key={s} value={s}>Shift {s}</option>)}
                                </select>
                              </td>
                              <td>
                                <select className="inline-select" value={m.rotationOffset!==undefined?m.rotationOffset:''} onChange={e=>updateMember(tk,m.id,'rotationOffset',e.target.value!==''?Number(e.target.value):undefined)}>
